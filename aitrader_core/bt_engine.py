...
class DataFeed:
    def __init__(self, task: Task, path='quotes'):
        dfs = CsvDataLoader().read_dfs(symbols=task.symbols, path=path, start_date=task.start_date, end_date=task.end_date)
...
class Engine:
    ...
    def _get_algos(self, task: Task):
        ...
        if algo_order_by:
            ...
            # 添加清仓逻辑：只保留selected中的持仓
            from bt_algos_extend import ClosePositionsNotSelected
            algos.append(ClosePositionsNotSelected())
    ...
    def run(self, task: Task, commissions=None):
        ...
        self.datafeed = DataFeed(task, path=self.path)
