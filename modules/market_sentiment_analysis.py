...
def display_market_sentiment_analysis():
    ...
    with st.expander("📖 功能说明", expanded=False):
        ...
        **情绪温度计算公式：**
        ```
        情绪温度 = 涨停占比得分(0-50) + 市场活跃度(0-30) + 连板强度(0-15) + 风险惩罚(-15-0) + 不活跃调整(-20)
        
        - 涨停占比得分 = (涨停数量 / (涨停+跌停)) × 50
        - 市场活跃度 = min(log(1+涨停数量) / log(51) × 30, 30)  # 使用对数函数，更平滑
        - 连板强度 = min(连板高度 / 8 × 15, 15)  
        - 风险惩罚 = max(-15, -log(1+跌停数量) / log(31) × 15)  # 使用对数函数，更平滑
        - 不活跃调整 = -20 (当涨停和跌停都为0时)
        ```
