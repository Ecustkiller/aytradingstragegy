...
def display_industry_fund_flow():
    ...
    # 调试信息
    st.write(f"🔍 调试信息：获取到 {len(df)} 条数据")
    st.write(f"🔍 调试信息：数据是否为空: {df.empty}")
    ...
    try:
        ...
        st.write(f"🔍 调试信息：列名映射后: {df.columns.tolist()}")
        ...
        st.write(f"🔍 调试信息：数据清洗完成，形状: {df.shape}")
    except Exception as e:
        st.error(f"数据处理阶段出错: {e}")
        import traceback
