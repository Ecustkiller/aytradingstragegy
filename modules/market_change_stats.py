...
def get_market_change_data(target_date: str, target_time: str) -> dict:
    """
    统计指定日期和时分的大盘涨跌幅情况
    ...
    try:
        ...
        if df is None or df.empty:
            logger.warning(f"未获取到数据: {target_date} {target_time}")
            return None
        ...
        logger.debug(f"选择的涨跌幅列: {selected_column}")
        ...
        def extract_concepts(concept_str):
            ...
            if pd.isna(concept_str):
                return []
            return [c.strip() for c in re.split(r'[,;，；]', str(concept_str)) if c.strip()]
        ...
        stats = {
            ...
            '涨停数量': len(limit_up),
            '跌停数量': len(limit_down),
            '涨停股票': limit_up[['股票代码', '股票简称', selected_column, '所属概念']].copy() if not limit_up.empty else pd.DataFrame(),
            '跌停股票': limit_down[['股票代码', '股票简称', selected_column, '所属概念']].copy() if not limit_down.empty else pd.DataFrame(),
            '涨停概念': pd.DataFrame(limit_up_concepts) if limit_up_concepts else pd.DataFrame(),
            '跌停概念': pd.DataFrame(limit_down_concepts) if limit_down_concepts else pd.DataFrame(),
            '区间分布': range_stats,
            '详细数据': valid_data[['股票代码', '股票简称', selected_column, '所属概念']].copy()
        }
        
        logger.info(f"统计完成: 总家数={total_count}, 涨停={len(limit_up)}, 跌停={len(limit_down)}")
        return stats
        
    except Exception as e:
        logger.error(f"统计过程中发生错误: {e}", exc_info=True)
        st.error(f"统计过程中发生错误: {e}")
        return None


def display_market_change_stats():
    """显示大盘涨跌统计界面"""
    st.title("📈 大盘涨跌统计")
