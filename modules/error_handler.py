...
def safe_execute(
    func: Callable, error_message: str = "执行失败", default_value: Any = None, log_error: bool = True
) -> Any:
    """
    安全执行函数，捕获异常并返回默认值
...
def validate_required(value: Any, name: str, error_message: Optional[str] = None) -> None:
    """
    验证必填参数
...
class ErrorContext:
    ...
    def __exit__(self, exc_type, exc_val, exc_tb):
        if exc_type is None:
            logger.info(
                f"{self.operation_name}完成: " f"成功 {self.success_count}, 失败 {self.error_count}"
            )
        else:
            logger.error(f"{self.operation_name}异常终止: {exc_val}")
        return False

    def execute(self, func: Callable[[], None], item_name: str = "项目") -> bool:
        """
        执行单个操作并记录结果
        ...
        try:
            ...
            self.error_count += 1
            error_info = {"item": item_name, "error": str(e), "traceback": traceback.format_exc()}
    ...
    def get_summary(self) -> str:
        """获取执行摘要"""
        return f"{self.operation_name}完成\n" f"成功: {self.success_count}\n" f"失败: {self.error_count}"

    def show_errors(self, max_display: int = 5) -> None:
        """在UI显示错误信息"""
...
handle_data_error = functools.partial(
    handle_errors, error_message="数据处理失败", show_traceback=False, return_value=None
)

handle_api_error = functools.partial(
    handle_errors, error_message="API调用失败", show_traceback=False, return_value=None
)

handle_file_error = functools.partial(
    handle_errors, error_message="文件操作失败", show_traceback=True, return_value=None
)
