...
def get_stock_data_ashare(symbol, start, end, period_type):
    ...
    try:
        ...
        # 转换日期格式
        start_str = start.strftime('%Y%m%d') if hasattr(start, 'strftime') else str(start).replace('-', '')
        end_str = end.strftime('%Y%m%d') if hasattr(end, 'strftime') else str(end).replace('-', '')
        ...
        if period_type == 'daily':
            df = get_price(formatted_symbol, start_date=start_str, end_date=end_str, frequency='1d', fields=['open', 'high', 'low', 'close', 'volume'])
        elif period_type == 'weekly':
            df = get_price(formatted_symbol, start_date=start_str, end_date=end_str, frequency='1w', fields=['open', 'high', 'low', 'close', 'volume'])
        elif period_type == 'monthly':
            df = get_price(formatted_symbol, start_date=start_str, end_date=end_str, frequency='1M', fields=['open', 'high', 'low', 'close', 'volume'])
        else:
